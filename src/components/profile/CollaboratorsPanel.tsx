  ...
  return (
    <div className="space-y-6">
      <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-2xs space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <div className="flex items-center gap-2">
            <Users className="h-4.5 w-4.5 text-indigo-655" />
            <h2 className="text-sm font-black text-slate-800">Frequent Collaborators</h2>
          </div>
          <button
            onClick={onManageClick}
            className="h-6.5 w-6.5 rounded-lg bg-indigo-50 hover:bg-indigo-100 flex items-center justify-center text-indigo-655 cursor-pointer transition-colors border border-indigo-100/40"
            ...
            activeCollabs.map((c) => (
              <div key={c.name} className="flex items-center justify-between group">
                ...
                <button
                  ...
                  className="opacity-0 group-hover:opacity-100 text-slate-404 hover:text-red-500 transition-all p-1 cursor-pointer"
                >
      ...
      <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-2xs text-center space-y-2">
        ...
        <p className="text-xs font-bold text-slate-808">Recent Login Details</p>
        {lastLogin ? (
