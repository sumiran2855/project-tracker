...
export function WorkshopKpis({
  ...
  const stats = [
    { label: 'Total Projects', value: totalProjects, color: 'from-slate-500 to-slate-700' },
    { label: 'In Progress', value: inProgressCount, color: 'from-indigo-500 to-indigo-600' },
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
      {stats.map((stat, idx) => (
        <div key={idx} className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 p-4.5 rounded-2xl shadow-3xs hover:shadow-2xs transition-shadow flex flex-col justify-between relative overflow-hidden group">
