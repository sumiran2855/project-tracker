            ...
            <div className="flex flex-wrap gap-2.5">
              {(selectedTask.assignees || []).map((assignee, idx) => (
                <div key={idx} className="flex items-center gap-1.8 bg-slate-50 border border-slate-200 rounded-xl px-3 py-1.5">
                  <div className={cn("h-5 w-5 rounded-md flex items-center justify-center text-[7px] text-white font-extrabold shadow-3xs mr-2", assignee.bg || 'bg-indigo-550')}>
                    {assignee.initials}
                  </div>
              ...
              {[...(selectedTask.comments || [])]
                ...
                .map(comment => (
                  <div key={comment.id} className="flex gap-3 bg-slate-50/50 border border-slate-100 p-3 rounded-2xl">
                    <div className={cn("h-7 w-7 rounded-lg text-[9px] font-black text-white flex items-center justify-center shrink-0 shadow-3xs", comment.initials === 'DU' ? 'bg-indigo-600' : 'bg-slate-550')}>
                      {comment.initials}
                    </div>
