      ...
      {columns.map(col => {
        ...
        return (
          <div
            ...
            className="bg-[#f8fafc] dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 flex flex-col min-h-[400px] shadow-2xs transition-colors hover:border-slate-300 dark:hover:border-slate-700"
          >
            {/* Column Header */}
            <div className="flex items-center justify-between mb-2 border-b border-slate-200 dark:border-slate-800 pb-3">
              <div>
                <h3 className="text-xs font-black text-slate-800 dark:text-slate-100 uppercase tracking-wider">{col.label}</h3>
                <p className="text-[9px] text-slate-400 dark:text-slate-500 font-bold mt-0.5 leading-none">{col.desc}</p>
              </div>
              <span className="rounded-full bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 text-[10px] font-black px-2.5 py-0.5 shadow-3xs">
            ...
            <div className="space-y-4 flex-1 pr-1.5 mt-2">
              {columnProjects.length === 0 ? (
                <div className="flex flex-col items-center justify-center p-8 border-2 border-dashed border-slate-200/80 dark:border-slate-800 rounded-2xl text-slate-350 dark:text-slate-500 text-[10px] font-extrabold text-center h-32 select-none">
                  Drag Projects Here
                </div>
              ) : (
                columnProjects.map(project => {
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        "group bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 rounded-2xl p-4 shadow-3xs hover:shadow-md transition-all duration-200 relative overflow-hidden",
                    ...
                    >
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          {!isEmployee && (
                            <button
                              ...
                              className="text-slate-400 dark:text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-400 p-1 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg transition-colors cursor-pointer"
                              title="Edit Dates"
                            >
                        ...
                        <p className="text-[10px] text-slate-400 dark:text-slate-450 line-clamp-2 leading-relaxed">
                          {project.description}
                        </p>
                      ...
                      <div className="border-t border-slate-100 dark:border-slate-800 pt-3 mt-4 flex items-center justify-between">
                        ...
                        <div className="flex -space-x-1.5 overflow-hidden">
                          {project.members.filter((m: any) => {
                            ...
                            <div key={i} className={cn("h-5 w-5 rounded-md text-[7px] font-bold text-white flex items-center justify-center ring-2 ring-white dark:ring-slate-950 shadow-3xs shrink-0", member.bg)} title={member.name}>
                              {member.initials}
                            </div>
                          ))}
                        </div>
